...
using LoginModel = IIT.EResult.Models.LoginModel;
using RegisterModel = IIT.EResult.Models.RegisterModel;
...
namespace IIT.EResult.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            else
            {
                if (Session["Mode"].ToString() == "1")
                {
                    Session["Mode"] = null;
                    ViewBag.Mode = true;
                    ViewBag.Message = "Registration Success. Please enter password to login.";
        ...
        public ActionResult Login(LoginModel loginModel, string returnUrl)
        {
            try
            {
                AuthStatus authStatus = _ERFormsAuthenticationService.SignIn(loginModel.UserName, loginModel.Password, false);
                if (authStatus.Code == AuthStatusCode.SUCCESS)
                {
                    //List<Employee> userModelList = new List<Employee>() { authStatus.Data as Employee };
                    ViewBag.Login = loginModel.UserName;
                    ViewBag.PIN = loginModel.Password;
        ...
        public ActionResult Register(RegisterModel registerModel)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var registerBiz = new RegisterBiz();
                    var duplicateUser = registerBiz.CheckDuplicateUser(registerModel);
                    ...
                    {
                        registerModel = registerBiz.SaveOrUpdate(registerModel);
                        if (registerModel.Id > 0)
                        {
                            Session["Mode"] = "1";
                            Session["Message"] = "Registration Success";
                            Session["Username"] = registerModel.UserName;
                            Session["Password"] = registerModel.Password;
